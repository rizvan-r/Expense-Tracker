...
export const HealthScore = ({ setActiveView }) => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-12">
      {/* Header Banner */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-emerald-900/30 via-indigo-900/30 to-slate-900/50 p-6 rounded-3xl border border-emerald-500/20 glass-panel">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Activity className="w-4 h-4 text-emerald-400" />
            <span className="text-xs font-semibold uppercase tracking-wider text-emerald-400">Financial Wellness Index</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white font-heading">AI Financial Health Score</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="flex flex-col items-center justify-center p-8 text-center relative overflow-hidden">
          <div className="relative w-48 h-48 flex items-center justify-center mb-4">
            ...
            <div className="absolute flex flex-col items-center justify-center text-center">
              ...
              <span className="text-xs uppercase tracking-widest text-slate-400 font-semibold mt-0.5">Score</span>
            </div>
          </div>

          <h3 className="text-lg font-bold text-white font-heading">{tier} Financial Wellness</h3>
        ...
        <Card className="lg:col-span-2 p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-bold text-white font-heading">Health Points Diagnostic Breakdown</h3>
            <span className="text-xs text-slate-400 font-mono">Base: 70 pts</span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Factor 1: Discretionary Negative Impact (Food, Misc, Subscriptions, Entertainment) */}
            <div className="p-4 bg-slate-900/60 rounded-xl border border-rose-500/20 space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="font-semibold text-rose-300 flex items-center gap-1">
                  🔻 Negative Deductions
                </span>
                <span className="text-rose-400 font-bold">-{breakdown.negative_deductions ?? breakdown.category_score ?? 4} pts</span>
              </div>
            ...
            <div className="p-4 bg-slate-900/60 rounded-xl border border-emerald-500/20 space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="font-semibold text-emerald-300 flex items-center gap-1">
                  🟢 Positive Boosters
                </span>
                <span className="text-emerald-400 font-bold">+{breakdown.positive_boost ?? breakdown.savings_score ?? 22} pts</span>
              </div>
              <div className="w-full bg-slate-800 rounded-full h-2">
            ...
            <div className="p-4 bg-slate-900/60 rounded-xl border border-slate-800 space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="font-semibold text-slate-200">Base Points Pool</span>
                <span className="text-indigo-400 font-bold">{breakdown.base_points || 70} / 70 pts</span>
              </div>
              <div className="w-full bg-slate-800 rounded-full h-2">
                <div className="bg-indigo-500 h-2 rounded-full" style={{ width: '100%' }}></div>
              </div>
              <p className="text-[11px] text-slate-400">Initial health pool score before category adjustments.</p>
            </div>

            {/* Factor 4: Net Health Formula */}
            <div className="p-4 bg-slate-900/60 rounded-xl border border-slate-800 space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="font-semibold text-slate-200">Net Health Points</span>
                <span className="text-amber-400 font-bold">{score} / 100 pts</span>
              </div>
              <div className="w-full bg-slate-800 rounded-full h-2">
                <div className="bg-amber-500 h-2 rounded-full" style={{ width: `${score}%` }}></div>
              </div>
              <p className="text-[11px] text-slate-400">Formula: Base (70) + Boost - Deductions</p>
            </div>
          </div>
        </Card>
      </div>
      ...
      <Card className="p-6 space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-bold text-white font-heading">AI Health Improvement Action Items</h3>
        </div>
        ...
        <div className="space-y-3">
          {(healthScoreData?.action_items || [
            ...
            <div key={index} className="flex items-start gap-3 p-3 bg-slate-900/50 rounded-xl border border-slate-800">
